...
function getPackageReportData(package, versionRangeOrCallback, callback) {
	...
	if (!versionRange) {
		...
		return callback(null, { name: package, version: message })
	}
	...
	getPackageJson(package, function(err, json) {
		...
		var versionBlob = json.versions[version];

		var authors = versionBlob.author ? versionBlob.author.name : null;

		if (!authors && versionBlob.authors) {
			authors = versionBlob.authors.join(', ');
		}
		...
		getPackageJson(package, version, function(err, json) {
			...
			*/
			callback(null, {
				...
				uri: extractLink(json),
				version: version.toString(),
				licenseHolder: authors,
			})
		})
	})
}
